...
void FlyByWireModelClass::step()
{
  ...
  FlyByWire_LagFilter(FlyByWire_U.in.data.alpha_deg, FlyByWire_P.LagFilter_C1, FlyByWire_U.in.time.dt, &rtb_Gain1_b,
  ...
  FlyByWire_CalculateV_alpha_max(FlyByWire_U.in.data.V_ias_kn, rtb_Gain1_b, rtb_Y, rtb_Y_k1, &rtb_Limiterxi);
  ...
  FlyByWire_CalculateV_alpha_max(FlyByWire_U.in.data.V_ias_kn, rtb_Gain1_b, rtb_Y, rtb_Y_c, &rtb_uDLookupTable_g);
  FlyByWire_RateLimiter(look2_binlxpw(FlyByWire_U.in.data.V_mach, FlyByWire_U.in.data.flaps_handle_index,
  ...
  FlyByWire_Y.out.sim.data_speeds_aoa.alpha_filtered_deg = rtb_Gain1_b;
  ...
  if ((rtb_BusAssignment_sim_input_delta_eta_pos >= -0.03125) || (rtb_Gain1_b >= rtb_Y_k1) ||
      (FlyByWire_DWork.resetEventTime == 0.0)) {
    FlyByWire_DWork.resetEventTime = FlyByWire_U.in.time.simulation_time;
  }

  if ((rtb_on_ground == 0) && (FlyByWire_U.in.data.autopilot_custom_on == 0.0) && (rtb_Gain1_b > rtb_Y_c) &&
  ...
  if ((FlyByWire_U.in.time.simulation_time - FlyByWire_DWork.resetEventTime > 0.5) ||
      (rtb_BusAssignment_sim_input_delta_eta_pos < -0.5) || ((FlyByWire_U.in.data.H_radio_ft < 200.0) &&
       (rtb_BusAssignment_sim_input_delta_eta_pos < 0.5) && (rtb_Gain1_b < rtb_Y_c - 2.0)) || (rtb_on_ground != 0)) {
  ...
  if ((rtb_alpha_floor_inhib == 0) && (FlyByWire_U.in.data.V_mach < 0.6)) {
    ...
    if ((rtb_Gain1_b > rtb_Y_h + std::fmin(std::fmax(rtb_Y_p, static_cast<real_T>(rtb_nz_limit_lo_g)), 0.0)) &&
        (FlyByWire_DWork.Delay_DSTATE > 10.0)) {
  ...
  FlyByWire_Y.out.sim.data_computed.protection_ap_disc = (((rtb_on_ground == 0) && (((rtb_ap_special_disc != 0) &&
    (rtb_Gain1_b > rtb_Y_k1)) || (rtb_Gain1_b > rtb_Y_c + 0.25))) || (FlyByWire_U.in.time.simulation_time -
  ...
  rtb_Gain1_b = (((((rtb_Gain_c - FlyByWire_DWork.Delay_DSTATE_dd) / FlyByWire_U.in.time.dt + FlyByWire_P.Gain_Gain_h *
  ...
  rtb_Divide_ke = rtb_Gain1_b / std::cos(FlyByWire_P.Gain1_Gain_b * rtb_Gain_im);
  ...
  FlyByWire_DWork.Delay1_DSTATE = 1.0 / rtb_Limiterxi * (FlyByWire_P.Constant_Value_m3 - rtb_Gain1_b) *
    FlyByWire_DWork.Delay1_DSTATE + (rtb_Divide_np + FlyByWire_DWork.Delay_DSTATE_c) * (rtb_Gain1_b / rtb_Limiterxi);
  ...
  FlyByWire_DWork.Delay1_DSTATE_i = 1.0 / rtb_Limiterxi * (FlyByWire_P.Constant_Value_hz - rtb_Gain1_b) *
    FlyByWire_DWork.Delay1_DSTATE_i + (rtb_Divide_a + FlyByWire_DWork.Delay_DSTATE_m) * (rtb_Gain1_b / rtb_Limiterxi);
  ...
  if (FlyByWire_U.in.data.autopilot_custom_on > FlyByWire_P.Switch1_Threshold_ke) {
    rtb_Gain1_b = (rtb_Y_mc5 - rtb_GainTheta) * FlyByWire_P.Gain4_Gain;
  } else {
    rtb_Gain1_b = FlyByWire_P.Gain1_Gain * rtb_GainTheta;
    rtb_Y_mc5 = rtb_Divide1 - rtb_Divide_ke;
    ...
    L_xi = FlyByWire_P.Gain2_Gain * rtb_Y_k - rtb_Gain1_b;
    ...
    rtb_Y_g = look1_binlxpw(L_xi, FlyByWire_P.Loaddemand1_bp01Data, FlyByWire_P.Loaddemand1_tableData, 2U) + rtb_Y_mc5;
    if (rtb_Loaddemand <= rtb_Y_g) {
      L_xi = FlyByWire_P.Gain3_Gain * FlyByWire_P.Theta_max3_Value - rtb_Gain1_b;
      ...
      rtb_Y_g = look1_binlxpw(L_xi, FlyByWire_P.Loaddemand2_bp01Data, FlyByWire_P.Loaddemand2_tableData, 2U) + rtb_Y_mc5;
      if (rtb_Loaddemand >= rtb_Y_g) {
        rtb_Y_g = rtb_Loaddemand;
      }
    }

    rtb_Gain1_b = rtb_Y_g + rtb_Switch_c;
  }

  rtb_Gain1_b += rtb_Divide_ke;
  if (rtb_Gain1_b > rtb_Y_jz) {
    rtb_Gain1_b = rtb_Y_jz;
  } else if (rtb_Gain1_b < rtb_Y_lc) {
    rtb_Gain1_b = rtb_Y_lc;
  }
  ...
  rtb_Divide_ke = std::fmax(std::fmax(0.0, rtb_GainTheta - 22.5), std::fmax(0.0, (std::abs(rtb_GainPhi) - 3.0) / 6.0));
  if ((!FlyByWire_DWork.pY_not_empty_m) || (!FlyByWire_DWork.pU_not_empty)) {
    FlyByWire_DWork.pU = rtb_Divide_ke;
    FlyByWire_DWork.pU_not_empty = true;
    FlyByWire_DWork.pY_p = rtb_Divide_ke;
    FlyByWire_DWork.pY_not_empty_m = true;
  }

  rtb_Y_k = FlyByWire_U.in.time.dt * FlyByWire_P.WashoutFilter_C1;
  rtb_Y_g = 2.0 / (rtb_Y_k + 2.0);
  FlyByWire_DWork.pY_p = (2.0 - rtb_Y_k) / (rtb_Y_k + 2.0) * FlyByWire_DWork.pY_p + (rtb_Divide_ke * rtb_Y_g -
    FlyByWire_DWork.pU * rtb_Y_g);
  FlyByWire_DWork.pU = rtb_Divide_ke;
  rtb_Limiterxi = (rtb_y_l - (rtb_Y_p - rtb_Y_c)) - FlyByWire_DWork.pY_p;
  ...
  rtb_Limiterxi = rtb_uDLookupTable_g - (look1_binlxpw(FlyByWire_U.in.data.V_tas_kn,
    ...
    + FlyByWire_P.Bias_Bias) * (rtb_Gain1_b - rtb_Divide1);
  ...
  rtb_Switch_c = FlyByWire_P.DiscreteDerivativeVariableTs2_Gain * FlyByWire_U.in.data.V_tas_kn;
  FlyByWire_LagFilter((rtb_Switch_c - FlyByWire_DWork.Delay_DSTATE_fi) / FlyByWire_U.in.time.dt,
  ...
  if (rtb_Y_p > FlyByWire_P.Saturation1_UpperSat_e) {
    rtb_Y_p = FlyByWire_P.Saturation1_UpperSat_e;
  } else if (rtb_Y_p < FlyByWire_P.Saturation1_LowerSat_m) {
    rtb_Y_p = FlyByWire_P.Saturation1_LowerSat_m;
  }
  ...
  rtb_Limiterxi = (((rtb_Loaddemand - FlyByWire_DWork.Delay_DSTATE_ca) / FlyByWire_U.in.time.dt *
    ...
    FlyByWire_P.PLUT_bp01Data, FlyByWire_P.PLUT_tableData, 1U)) + (rtb_Divide1 - FlyByWire_DWork.Delay_DSTATE_jv) /
                   FlyByWire_U.in.time.dt) + FlyByWire_P.Gain_Gain_o * rtb_Y_p;
  ...
  rtb_Gain1_b = FlyByWire_P.DiscreteTimeIntegratorVariableTs_Gain_k * rtb_alpha_err_gain * FlyByWire_U.in.time.dt;
  ...
  if (FlyByWire_DWork.icLoad_e) {
    ...
    FlyByWire_DWork.Delay_DSTATE_f1 = L_xi - rtb_Gain1_b;
  }
  ...
  rtb_uDLookupTable_g = FlyByWire_DWork.Delay_DSTATE_f1 * rtb_Gain1_b;
  rtb_Y_fp = FlyByWire_P.Constant_Value_o - rtb_Gain1_b;
  if (rtb_ManualSwitch > FlyByWire_P.Saturation_UpperSat_c) {
    ...
    rtb_Gain1_b = rtb_ManualSwitch;
  }

  rtb_LimiteriH = ((FlyByWire_P.Constant_Value_ju - rtb_Gain1_b) *
                   rtb_BusAssignment_cs_pitch_data_computed_delta_eta_deg + rtb_Limitereta * rtb_Gain1_b) * rtb_Y_fp +
    rtb_uDLookupTable_g;
  ...
  rtb_Gain1_b = FlyByWire_P.Gain_Gain_ip * rtb_uDLookupTable_g * FlyByWire_P.DiscreteTimeIntegratorVariableTsLimit_Gain *
  ...
  if (FlyByWire_DWork.icLoad_i) {
    FlyByWire_DWork.Delay_DSTATE_h = rtb_eta_trim_deg_reset_deg - rtb_Gain1_b;
  }
  ...
  if (rtb_Gain1_o > rtb_uDLookupTable_g) {
    rtb_Gain1_o = rtb_uDLookupTable_g;
  } else {
  ...
  rtb_Gain1_b = FlyByWire_P.Gain1_Gain_bq * rtb_BusAssignment_sim_input_delta_xi_pos + rtb_uDLookupTable_g;
  if (rtb_Gain1_b > FlyByWire_P.Saturation_UpperSat_as) {
    rtb_Gain1_b = FlyByWire_P.Saturation_UpperSat_as;
  ...
  rtb_uDLookupTable_g = std::fmin(rtb_uDLookupTable_g, std::fmax(rtb_Limiterxi, rtb_Gain1_b * rtb_Y_ply)) *
  ...
  FlyByWire_RateLimiter(rtb_Gain1_o, FlyByWire_P.RateLimiterVariableTs_up_i, FlyByWire_P.RateLimiterVariableTs_lo_g,
  ...
  FlyByWire_RateLimiter(static_cast<real_T>(rtb_on_ground), FlyByWire_P.RateLimiterVariableTs_up_f1,
                        ...
                        FlyByWire_P.RateLimiterVariableTs_InitialCondition_fa, &rtb_Gain1_b,
                        &FlyByWire_DWork.sf_RateLimiter_f);
  if (rtb_Gain1_b > FlyByWire_P.Saturation_UpperSat_cr) {
    rtb_Gain1_b = FlyByWire_P.Saturation_UpperSat_cr;
  } else if (rtb_Gain1_b < FlyByWire_P.Saturation_LowerSat_o4) {
    rtb_Gain1_b = FlyByWire_P.Saturation_LowerSat_o4;
  }

  rtb_uDLookupTable_g = FlyByWire_U.in.data.autopilot_custom_Beta_c_deg * rtb_Gain1_b;
  rtb_Limiterxi = FlyByWire_P.Constant_Value_i - rtb_Gain1_b;
  if (FlyByWire_U.in.data.autopilot_custom_on > FlyByWire_P.Switch2_Threshold_n) {
    rtb_Gain1_b = FlyByWire_U.in.data.autopilot_custom_Beta_c_deg + rtb_Y_fp;
  } else {
    rtb_Gain1_b = rtb_BusAssignment_sim_input_delta_zeta_pos * look1_binlxpw(FlyByWire_U.in.data.V_ias_kn,
  ...
  FlyByWire_LagFilter((rtb_Gain1_b - L_xi) * look1_binlxpw(FlyByWire_U.in.data.V_ias_kn,
  ...
  L_xi = rtb_Gain1_b * look1_binlxpw(FlyByWire_U.in.data.V_ias_kn, FlyByWire_P.ScheduledGain_BreakpointsForDimension1_c,
  ...
  if (rtb_Limiterxi > FlyByWire_P.Saturation1_UpperSat_eh) {
    rtb_Gain1_b = FlyByWire_P.Saturation1_UpperSat_eh;
  } else if (rtb_Limiterxi < FlyByWire_P.Saturation1_LowerSat_l) {
    rtb_Gain1_b = FlyByWire_P.Saturation1_LowerSat_l;
  } else {
    rtb_Gain1_b = rtb_Limiterxi;
  }
  ...
  rtb_Y_p = (FlyByWire_P.Constant_Value_l - rtb_Gain1_b) * rtb_Y_nl + FlyByWire_DWork.Delay_DSTATE_eu * rtb_Gain1_b;
  ...
  rtb_Limiterxi = (FlyByWire_P.Constant_Value_f - rtb_Gain1_b) * rtb_Gain1_o + rtb_uDLookupTable_g * rtb_Gain1_b;
  ...
  rtb_Gain1_b = FlyByWire_P.Gain4_Gain_e * L_xi * FlyByWire_P.DiscreteTimeIntegratorVariableTs1_Gain *
    FlyByWire_U.in.time.dt;
  ...
  if (FlyByWire_DWork.icLoad_d) {
    FlyByWire_DWork.Delay_DSTATE_f3 = rtb_BusAssignment_a_sim_data_zeta_trim_deg - rtb_Gain1_b;
  }

  FlyByWire_DWork.Delay_DSTATE_f3 += rtb_Gain1_b;
  ...
  FlyByWire_DWork.Delay_DSTATE_fi = rtb_Switch_c;
